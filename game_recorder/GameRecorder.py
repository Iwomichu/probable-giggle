...
class GameRecorder:
    def __init__(self, screen_width: int, screen_height: int, grayscale: bool = False, filename: str = "test.mp4", frame_length: int = 1):
    ...
    def add_torch_frame(self, frame_torch: torch.Tensor):
        self.add_frame(frame_torch.transpose(2, 0).numpy())

    def save_recording(self) -> None:
        fps = 20
        out = cv2.VideoWriter(self.filename, cv2.VideoWriter_fourcc(*'mp4v'), fps, (self.width, self.height), not self.grayscale)
