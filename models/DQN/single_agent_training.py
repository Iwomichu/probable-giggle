...
from models.DQN.domain_types import HasAgentWon, GameLength, RawAction, State

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def train_model(
        env: SpaceGameEnvironment = None,
    ...
    save_models_directory = save_models_directory if save_models_directory else SAVED_MODELS_DIRECTORY / train_run_id

    dqn_config = dqn_config if dqn_config is not None else Config.default()

    if env is None:
        env_config = SpaceGameEnvironmentConfig.default()
        env = SpaceGameEnvironment(env_config)
...
def test(dqn_config: Config, env: SpaceGameEnvironment, recordings_directory: Path, target_net: DQN,
         writer: SummaryWriter, test_episode_count: int):
    games_won_lengths = []
    games_lost_lengths = []
...
def train(env: SpaceGameEnvironment, dqn_config: Config, policy_net: DQN, n_actions: int, memory: ReplayMemory,
    ...
    for t in range(3000):
        ...
        if not done:
            ...
            next_state = None
        memory.push(state.cpu().item(), action, next_state.cpu().item() if next_state is not None else None, reward)
        state = next_state
